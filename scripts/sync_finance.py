...
Only whole rows are ever appended. Nothing already in the sheet is edited,
moved or deleted - Thuan's own tabs are never touched.
"""
...
def main():
    ...
    pending = []
    for d in db.collection("expenses").where(filter=FieldFilter("status", "==", "new")).stream():
        e = d.to_dict() or {}
        e["_ref"] = d.reference
        e["_id"] = d.id
        pending.append(e)
